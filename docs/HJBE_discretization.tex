...
\paragraph{Iterative Method }Pick an $h$, which is used in the iterative process.\footnote{In effect, I think that this is like using an explicit time step in a PDE, where we are forward iterating fixing the policy?  Because of this, there is a high likelihood that it is not unconditionally stable, like other explicit PDE methods in time.}  We first make an initial guess $v^0 = (v_1^0, \dots v_I^0)$. Then for each consecutive iteration $n = 0, 1, 2, ...$,  update $v^n$ using the following equation
\begin{align}
...
When $v^{n+1}$ is sufficiently close in value to $v^n$, the algorithm is complete. Otherwise, update the value of $v^n$ and repeat the previous steps for the next iteration. Keep in mind that the solution may not be unconditionally stable for an arbitrary $h$.\footnote{That is, there good chance that there is some sort of a Courant, Friedrichs, Lewy type condition required.}
