...
\begin{align}
	...
	\intertext{There also exists a steady state level of capital, $k^*$, given by}
	{\frac{}{\rho + \delta}}^{\frac{1}{1-\gamma}}
...
\begin{align}
	\frac{v_i^{n+1}-v_i^{n}}{\Delta} + \rho v_i^{n+1} = u(c_i^n) + (v_i^{n+1})^{'}  [f(k_i) - \delta k_i - c_i^n\label{eq:fd-approx-capital}]\	
\end{align}
...
\begin{align}
\mu_{i,F} = f(k_i) - \delta - \left(v_F'(k_i) \right)^{-\frac{1}{\gamma}}\\
\mu_{i,B} = f(k_i) - \delta - \left(v_B'(k_i) \right)^{-\frac{1}{\gamma}}\\
\end{align}
