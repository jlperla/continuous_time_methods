...
\mathcal{A} v(t_n, x_i) &\approx \underbrace{\left(v^n_i-v^n_{i-1}\right)\frac{\mu_i^{n, -}}{\Delta_{i,-}}+ \left(v^n_{i+1}-v^n_i\right)\frac{\mu_i^{n, +}}{\Delta_{i,+}}}_{\text{Upwind Scheme}}  + (\sigma^n_i)^2 \frac{\Delta_{i,-}v^n_{i+1} - (\Delta_{i,+}+\Delta_{i,-}) v^n_i + \Delta_{i,+}v^n_{i-1}}{\Delta_{i,-}\Delta_{i,+}(\Delta_{i,+}+\Delta_{i,-})}+ \frac{v^{n+1}_i - v^n_i}{h_{n,+}}\label{eq:A-generator-univariate-diffusion-with-nonuniform-grid-interior-t}
\end{align}
...
What will require a little more care is handling the $n=1$ and $n=N$ cases.  Hopefully, using \cref{eq:D-steady-state-T}, it just says that $\frac{v_i^N+1 - v_i^N}{h} = 0$, and hence the last term of these just drop out.
