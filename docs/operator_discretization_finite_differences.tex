...
\end{align}
In terms of the structure of \ref{eq:A} and \ref{eq:b}, we can similarly write\footnote{\textbf{TODO:} Check what the multiplication should be by comparing to the equations at each node.
}
Define
\begin{align}
...
\vdots\\
\Delta_{-}\\
\end{bmatrix}\in\R^{I\times I}\\
\intertext{Then, using pointwise multiplication}
\mathbf{\Delta}\odot \mathcal{A}v(\set{x_i}) &\approx A v
\end{align}
