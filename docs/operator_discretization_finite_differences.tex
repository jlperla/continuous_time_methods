...
\end{align}
We treat $\Delta_{1, -} = \Delta_{1, +}$ and $\Delta_{I, +} = \Delta_{I, -}$, due to ghost points, $v_0$ and $v_{I+1}$ on both boundaries.  \\
...
In the interior ($1 < i < I$), the discretization of \cref{eq:A-generator-univariate-diffusion}
\begin{align}
\Delta_{-} \mathcal{A} v(x_i) &\approx \underbrace{\left(v_i-v_{i-1}\right)\mu_i^{-}+ \left(v_{i+1}-v_i\right)\frac{\Delta_{-}}{\Delta_{+}}\mu_i^{+}}_{\text{Upwind Scheme}}  + \sigma_i^2 \frac{\Delta_{-}v_{i+1} - (\Delta_{+}+\Delta_{-}) v_i + \Delta_{+}v_{i-1}}{\Delta_{+}(\Delta_{+}+\Delta_{-})}\label{eq:A-generator-univariate-diffusion-with-nonuniform-grid-interior}\\
...
&= \underbrace{\left(-\mu_i^{-} +\frac{\sigma_i^2}{\Delta_{+}+\Delta_{-}}\right)}_{\equiv X_i}v_{i-1} + \underbrace{\left(\mu_i^{-} - \mu_i^{+}\frac{\Delta_{-}}{\Delta_{+}}-\frac{\sigma_i^2}{\Delta_{+}}\right)}_{\equiv Y_i}v_i + \underbrace{\left(\mu_i^{+}\frac{\Delta_{-}}{\Delta_{+}} + \frac{\Delta_{-}}{\Delta_{+}}\frac{\sigma_i^2}{\Delta_{+}+\Delta_{-}}\right)}_{\equiv Z_i}v_{i+1}\label{eq:A-collected-interior-with-nonuniform-grid}
\end{align}

\paragraph{Boundary Value at $\underline{x}$:}
As $i =1$, given the ``ghost node'' $x_0$, the discretized operator from \cref{eq:A-collected-interior-with-nonuniform-grid} is
\begin{align}
\Delta \mathcal{A} v(x_1) &\approx \left(-\mu_1^{-} +\frac{\sigma_1^2}{\Delta_{+}+\Delta_{-}}\right)v_0 + \left(\mu_1^{-} - \mu_1^{+}\frac{\Delta_{-}}{\Delta_{+}}-\frac{\sigma_1^2}{\Delta_{+}}\right) v_1 + \left(\mu_1^{+}\frac{\Delta_{-}}{\Delta_{+}} + \frac{\Delta_{-}}{\Delta_{+}}\frac{\sigma_1^2}{\Delta_{+}+\Delta_{-}}\right)v_2\label{eq:A-collected-with-nonuniform-grid-left}\\
\intertext{In the case of the boundary value $v(\underline{x}) = \underline{v}$, subsitute for $v_0 =  \underline{v}$ to find,}
&\approx \underbrace{\overbrace{\left(-\mu_1^{-} +\frac{\sigma_1^2}{\Delta_{+}+\Delta_{-}}\right)}^{\equiv X_1}\underline{v}}_{\equiv b_1} +  \underbrace{\left(\mu_1^{-} - \mu_1^{+}\frac{\Delta_{-}}{\Delta_{+}}-\frac{\sigma_1^2}{\Delta_{+}}\right)}_{\equiv Y_1}v_1 + \underbrace{\left(\mu_1^{+}\frac{\Delta_{-}}{\Delta_{+}} + \frac{\Delta_{-}}{\Delta_{+}}\frac{\sigma_1^2}{\Delta_{+}+\Delta_{-}}\right)}_{\equiv Z_1}v_2\\
...
\Delta \mathcal{A} v(x_1) &\approx \left(\underbrace{\left(-\mu_1^{-} +\frac{\sigma_1^2}{\Delta_{+}+\Delta_{-}}\right)}_{\equiv X_1} + \underbrace{\left(\mu_1^{-} - \mu_1^{+}\frac{\Delta_{-}}{\Delta_{+}}-\frac{\sigma_1^2}{\Delta_{+}}\right)}_{\equiv Y_1}\right)v_1 + \underbrace{\left(\mu_1^{+}\frac{\Delta_{-}}{\Delta_{+}} + \frac{\Delta_{-}}{\Delta_{+}}\frac{\sigma_1^2}{\Delta_{+}+\Delta_{-}}\right)}_{\equiv Z_1}v_2
\end{align}

\paragraph{Boundary Value at $\bar{x}$:}
As $i=I$, given the ``ghost node'' $x_{I+1}$, from \cref{eq:A-collected-interior}
\begin{align}
\Delta \mathcal{A} v(x_I)&\approx \left(-\mu_I^{-} +\frac{\sigma_I^2}{\Delta_{+}+\Delta_{-}}\right)v_{I-1} + \left(\mu_I^{-} - \mu_I^{+}\frac{\Delta_{-}}{\Delta_{+}}-\frac{\sigma_I^2}{\Delta_{+}}\right)v_I + \left(\mu_I^{+}\frac{\Delta_{-}}{\Delta_{+}} + \frac{\Delta_{-}}{\Delta_{+}}\frac{\sigma_I^2}{\Delta_{+}+\Delta_{-}}\right)v_{I+1}\\
\intertext{For the absorbing barrier, substitute for $v(x_{I+1}) = \bar{v}$,}
\Delta \mathcal{A} v(x_I)&\approx \underbrace{\left(-\mu_I^{-} +\frac{\sigma_I^2}{\Delta_{+}+\Delta_{-}}\right)}_{\equiv X_I} v_{I-1} + \underbrace{\left(\mu_I^{-} - \mu_I^{+}\frac{\Delta_{-}}{\Delta_{+}}-\frac{\sigma_I^2}{\Delta_{+}}\right)}_{\equiv Y_I} v_I + \underbrace{\overbrace{\left(\mu_I^{+}\frac{\Delta_{-}}{\Delta_{+}} + \frac{\Delta_{-}}{\Delta_{+}}\frac{\sigma_I^2}{\Delta_{+}+\Delta_{-}}\right)}^{\equiv Z_I}\bar{v}}_{\equiv b_I}\\
\intertext{For a reflecting barrier, the boundary value $v'(\bar{x}) \approx \frac{v_{I+1}-v_I}{\Delta} = 0, \implies v_{I+1} = v_I$,}
\Delta \mathcal{A} v(x_I)&\approx \underbrace{\left(-\mu_I^{-} +\frac{\sigma_I^2}{\Delta_{+}+\Delta_{-}}\right)}_{\equiv X_I} v_{I-1} + \left(\underbrace{\left(\mu_I^{-} - \mu_I^{+}\frac{\Delta_{-}}{\Delta_{+}}-\frac{\sigma_I^2}{\Delta_{+}}\right)}_{\equiv Y_I} + \underbrace{\left(\mu_I^{+}\frac{\Delta_{-}}{\Delta_{+}} + \frac{\Delta_{-}}{\Delta_{+}}\frac{\sigma_I^2}{\Delta_{+}+\Delta_{-}}\right)}_{\equiv Z_I} \right) v_I 
\end{align}
\paragraph{Simple Case with $\mu(x) < 0$ and Reflecting Barrier}
In the special case of $\mu(\bar{x}) < 0$, the upwind direction is trivial: $\mu^{-} = \mu$ and $\mu^{+} = 0$.  Specializing \cref{eq:X-non-uniform,eq:Y-non-uniform,eq:Z-non-uniform},
\begin{align}
	X &= - \mu+ \frac{\sigma^{2}}{\Delta_{+}+\Delta_{-}}\label{eq:X-non-uniform-backwards} \\
	Y &= \mu - \frac{\sigma^{2}}{\Delta_{+}}\label{eq:Y-non-uniform-backwards} \\
	Z &= \frac{\Delta_{-}}{\Delta_{+}}\frac{\sigma^{2}}{\Delta_{+}+\Delta_{-}}\label{eq:Z-non-uniform-backwards}
\end{align}
and the formula for the discretized operator is,
\begin{align}
