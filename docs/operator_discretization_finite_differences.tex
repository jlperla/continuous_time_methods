...
\paragraph{As an Eigenvalue Problem} Note that \cref{eq:linear-operator-stationary} is an eigenvalue problem.  Therefore, we can muliply by $\Delta$ and then the stationary distribution can be found as the eigenvector of $A^T$ associated with the zero eigenvalue, and rescales to ensure \cref{eq:linear-operator-sum} holds.  This is useful since $A^T$ is always singular, and hence \cref{eq:linear-operator-stationary} cannot be solved as a simple linear system.

\paragraph{As a Linear-Least Squares Problem} Alternatively, consider solving \cref{eq:linear-operator-stationary,eq:linear-operator-sum},after multiplying everything by $\Delta$, as the following linear least squares problem, subject to $f \geq 0$ inequality constraint.
