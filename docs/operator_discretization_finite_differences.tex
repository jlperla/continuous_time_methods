...
Here is an attempt for this non-uniform grid issue.\\
For non-uniform grid, $\Delta_{i, +} \equiv x_{i+1} - x_{i}$ and $\Delta_{i, -} \equiv x_{i} - x_{i-1}$\\
...
\end{align}
For the stability of numerical calculation, we can multiply every term in $A$ by $\Delta_{+}\Delta_{-}$ to obtain $\Delta_{+}\Delta_{-}A$. So we can redefine $X, Y $and $Z$ as 
\begin{align}
X &= -\mu^{-}\Delta_{+} + \frac{\sigma^2}{2}\frac{\Delta_{+}}{\frac{1}{2}(\Delta_{+}+\Delta_{-})} \label{eq:X non-nuiform}\\
Y &= -\mu^{+}\Delta_{-}+\mu^{-}\Delta_{+} -\sigma^2 \label{eq:Y non-nuiform}\\
Z &=\mu^{+}\Delta_{-} + \frac{\sigma^2}{2}\frac{\Delta_{-}}{\frac{1}{2}(\Delta_{+}+\Delta_{-})} \label{eq:Z non-nuiform}
\end{align}
It is not clear that how to deal with $\Delta_{1, -} = v_1 - v_0$ and $\Delta_{I, +} = v_{I+1} - v_I$, where $v_0$ and $v_I$ are not well defined. I temporarily treat $\Delta_{1, -} = \Delta_{1, +}$ and $\Delta_{I, +} = \Delta_{I, -}$. \\
There is another way to define $\Delta$ for Kolmogorov Forward Equation with non-uniform grids, which is introduced in \url{http://www.princeton.edu/~moll/HACTproject/HACT_Numerical_Appendix.pdf}. It defines $\tilde{\Delta}$ as an approximation as following
\begin{equation}
\tilde{\Delta} = 
\begin{cases}
&\frac{1}{2}\Delta_{i, +} \qquad i = 1\\ 
&\frac{1}{2}(\Delta_{i, +}+\Delta_{i, -}) \qquad i = 2, ..., I-1\\
&\frac{1}{2}\Delta_{i, -} \qquad i = I
\end{cases} \label{eqs:Delta_tilde}
\end{equation}

\noindent If this attempt is correct, care will need to be taken to determine the result from discretize\_ univariate\_diffusion.m , which is uniform grid or non-uniform grid whenever we are using the function discretize\_univariate\_diffusion.m.
